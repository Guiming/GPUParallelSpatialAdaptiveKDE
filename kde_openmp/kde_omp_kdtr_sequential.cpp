...
inline float GaussianKernel(float h2, float d2){
	...
	if(BRUNSDON){
		...
		return expf(d2 / (-2.0 * h2)) / (h2*TWO_PI); // Guassian
...
float LogLikelihoodKDtr(AsciiRaster Ascii, SamplePoints Points, float *edgeWeights, float h, bool update = true, int nThreads = 1);
...
float LogLikelihoodAdaptiveKDtr(AsciiRaster Ascii, SamplePoints Points, float *edgeWeights, float* hs, bool update = true, float* den0 = NULL, float* den1 = NULL, int nThreads = 1);
...
//void BuildKDtree (SamplePoints Points, KDtree *tree);
//void BuildKDtree (SamplePoints Points, vector <Point> &data);
void BuildKDtree (SamplePoints Points);
void SearchRange(KDtree const* tree, const Point &query, const float range, vector<int> &ret_index, vector<float> &ret_sq_dist);
...
/* declare the K-D tree as a global variable
 * be very careful with these two global variables
*/
...
int main(int argc, char *argv[]){
	...
	// By Guiming @ 2016-09-07
	// build a K-D tree on the sample points
	double start_kd = omp_get_wtime();
	BuildKDtree(Points);
	double stop_kd = omp_get_wtime();
	printf("#building KD tree on %d points took %f ms\n", Points.numberOfPoints, (stop_kd - start_kd) * 1000);


	if(false){ // test nearest neighbor search
		double ttotalkd = 0.0;
		double ttotalbrute = 0.0;
		for(int x = 0; x < Points.numberOfPoints; x++){
			Point query = data[x];

			// kd tree search
			int ret_index0 = -1;
			float ret_dist0 = FLT_MAX;
			start_kd = omp_get_wtime();
			tree.Search(query, &ret_index0, &ret_dist0);
			stop_kd = omp_get_wtime();
			ttotalkd += (stop_kd - start_kd);
			float dist0 = Distance2(query.coords[0], query.coords[1], Points.xCoordinates[ret_index0], Points.yCoordinates[ret_index0]);
		  //printf("best_idx: %2d best_dist: %f dist: %f x: %f y: %f\n", ret_index0, ret_dist0, dist0, Points.xCoordinates[ret_index0], Points.yCoordinates[ret_index0]);

			// brute force search
			float dis = 0;
			int ret_index = -1;
			float ret_dist = FLT_MAX;
			double start_brute = omp_get_wtime();
			for(int i = 0; i < Points.numberOfPoints; i++){
				dis = Distance2(query.coords[0], query.coords[1], Points.xCoordinates[i], Points.yCoordinates[i]);
				if(dis < ret_dist){
					ret_dist = dis;
					ret_index = i;
				}
			}
			double stop_brute = omp_get_wtime();
			ttotalbrute += (stop_brute - start_brute);
			float dist = Distance2(query.coords[0], query.coords[1], Points.xCoordinates[ret_index], Points.yCoordinates[ret_index]);
			//printf("best_idx: %2d best_dist: %f dist: %f x: %f y: %f\n\n", ret_index, ret_dist, dist, Points.xCoordinates[ret_index], Points.yCoordinates[ret_index]);

			if(ret_index0 != ret_index)
				printf("results from KD tree do not match that from brute force!\n");
			}
		printf("NEAREST NEIGHBOR KD: %.5f BRUTE: %.5f KD tree runs %.5f times faster! \n", ttotalkd, ttotalbrute, ttotalbrute/ttotalkd);
	}

	if(true){ // test reange search

		double ttotalkd = 0.0;
		double ttotalbrute = 0.0;
		for(int x = 0; x < 100; x++){

			int idx = Points.numberOfPoints*(rand() / (1.0 + RAND_MAX));

			Point query = data[idx];

			// kd tree search
			vector<int> ret_index0 = vector<int>();
			vector<float> ret_dist0 = vector<float>();
			float range = 6.06 * 6.06;
			start_kd = omp_get_wtime();
			tree.SearchRange(query, range, ret_index0, ret_dist0);
			stop_kd = omp_get_wtime();
			ttotalkd += (stop_kd - start_kd);
			...
			double start_brute = omp_get_wtime();
			tree.SearchRangeBruteForce(query, range, ret_index, ret_dist);
			double stop_brute = omp_get_wtime();
			ttotalbrute += (stop_brute - start_brute);

			if(ret_index0.size() != ret_index.size()){
				printf("results from KD tree do not match that from brute force!\n");
			}

		}
		printf("#RANGE SEARCH KD: %.5f BRUTE: %.5f KD tree runs %.5f times faster! \n", ttotalkd, ttotalbrute, ttotalbrute/ttotalkd);
	...
	*/
	...
	if(SKIPSEQ == 0){
		...
		if(Hoption == 2){
			...
			LogLikelihoodAdaptiveKDtr(Mask, Points, edgeWeights, bandWidths, true, den0, NULL);
...
//////////////////////////  START OF PARALLEL //////////////////////////
	if(SKIPPARA == 0){
		...
		if(Hoption == 2){
			...
			LogLikelihoodAdaptiveKDtr(Mask, Points, edgeWeights, bandWidths, true, den0, NULL, NUM_THREADS);
			float gml = compGML(den0, numPoints);
			for(int i = 0; i < numPoints; i++){
				bandWidths[i] = h0 * powf(den0[i]/gml, alpha0);
			}
...
float MLE_FixedBandWidth(AsciiRaster Ascii, SamplePoints Points, float *edgeWeights, float h, int nThreads){
	...
	while(condition){
		...
		float LoghB = LogLikelihoodKDtr(Ascii, Points, edgeWeights, hB, true, nThreads);
		float LoghC = LogLikelihoodKDtr(Ascii, Points, edgeWeights, hC, true, nThreads);
...
// the log likelihood given bandwidth h, use KD-tree for range query (find neighboring points within a radius)
// By Guiming @ 2016-09-16
float LogLikelihoodKDtr(AsciiRaster Ascii, SamplePoints Points, float *edgeWeights, float h, bool update, int nThreads){
	int numPoints = Points.numberOfPoints;
	float logL = 0.0f; // log likelihood
	float h2 = h * h;
	float range = CUT_OFF_FACTOR * h2;

	float* den1 = AllocateDen(Points);
	...
	for(int i = 0; i < numPoints; i++){
		den1[i] = -1.0 * GaussianKernel(h2, 0.0f) *  Points.weights[i] * edgeWeights[i];
		//den1[i] = 0.0f;
	}
	...
	//#pragma omp parallel for schedule(OMP_SCHEDULE_PNTS) num_threads(nThreads) private(ret_index, ret_dist)
	for(int i = 0; i < numPoints; i++){
		float pi_x = Points.xCoordinates[i];
		float pi_y = Points.yCoordinates[i];
		...
		SearchRange(&tree, query, range, ret_index, ret_dist);
		float g = 0.0f;
		int idx;
	...
	//#pragma omp parallel for num_threads(nThreads) reduction(+: logL)
	for(int i = 0; i < numPoints; i++){
		logL += logf(den1[i] + EPSILONDENSITY);

		if(BRUNSDON){
			logL += log(den1[i]/(numPoints - 1) + EPSILONDENSITY);
		}
	}
...
// float* den0 : density based on all points, including itself
// float* den1 : leave one out density
float LogLikelihoodAdaptiveKDtr(AsciiRaster Ascii, SamplePoints Points, float *edgeWeights, float* hs, bool update, float* den0, float* den1, int nThreads){
	int numPoints = Points.numberOfPoints;
	float logL = 0.0; // log likelihood

	float* denTmp = AllocateDen(Points);
	...
	//#pragma omp parallel for schedule(OMP_SCHEDULE_PNTS) num_threads(nThreads) reduction(+:logL) private(ret_index, ret_dist)
	for(int i = 0; i < numPoints; i++){
		float pi_x = Points.xCoordinates[i];
		float pi_y = Points.yCoordinates[i];
		float pj_w = Points.weights[i];
	...
	for(int i = 0; i < numPoints; i++){
		...
		if(BRUNSDON){
			logL += log(denTmp[i]/(numPoints - 1) + EPSILONDENSITY);
		}
	}
...
void hj_likelihood(AsciiRaster Ascii, SamplePoints Points, float *edgeWeights, float h0, float alpha0, float stepH, float stepA, int lastdmax, float* logLs, int nThreads){
    ...
    if(lastdmax == -1){ // avoid unnecessary [expensive] computation
	    ...
	    LogLikelihoodAdaptiveKDtr(Ascii, Points, edgeWeights, hs, true, den0, NULL, nThreads);
	    gml = compGML(den0, n);
	    for(int i = 0; i < n; i++){
	    	hs[i] = hs[i] * powf((den0[i] / gml), alpha0);
	    }
	    float L0 = LogLikelihoodAdaptiveKDtr(Ascii, Points, edgeWeights, hs, true, NULL, NULL, nThreads);
	    logLs[0] = L0;
	}
    ...
    if(lastdmax != 2){ // avoid unnecessary [expensive] computation
	    ...
	    float L1 = LogLikelihoodAdaptiveKDtr(Ascii, Points, edgeWeights, hs, true, NULL, NULL, nThreads);
	    logLs[1] = L1;
	}
    ...
    if(lastdmax != 1){
	    ...
	    float L2 = LogLikelihoodAdaptiveKDtr(Ascii, Points, edgeWeights, hs, true, NULL, NULL, nThreads);
	    logLs[2] = L2;
	}
    ...
    if(lastdmax != 4){
	    ...
	    float L3 = LogLikelihoodAdaptiveKDtr(Ascii, Points, edgeWeights, hs, true, NULL, NULL, nThreads);
	    logLs[3] = L3;
	}
    ...
    if(lastdmax != 3){
	    ...
	    float L4 = LogLikelihoodAdaptiveKDtr(Ascii, Points, edgeWeights, hs, true, NULL, NULL, nThreads);
	    logLs[4] = L4;
	}
...
//#pragma omp parallel for schedule(OMP_SCHEDULE_CELLS) num_threads(nThreads)
	for(int row = 0; row < Ascii.nRows; row++){
		for(int col = 0; col < Ascii.nCols; col++){
			...
			// cells in the outmost rows and cols
					if(Ascii.elements[row * Ascii.nCols + col] != Ascii.noDataValue){
						Ascii.elements[row * Ascii.nCols + col] = 1.0f;
						continue;
					}
			}
			else{ // cells in the interior
...
//#pragma omp parallel for schedule(OMP_SCHEDULE_PNTS) num_threads(nThreads) private(p_x, p_y, cell_x, cell_y)
	for (int p = 0; p < Points.numberOfPoints; p++){
...
{
    //printf("%f\n", range);
    vector <Point> &pts = *(tree->m_pts);
    vector <KDNode*> to_visit;
    KDNode *cur = tree->m_root;
    int m_cmps = 0;
