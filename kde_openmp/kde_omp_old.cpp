 ...
 *
 *         ./kde_omp [mode] [points_file] [mask_file] [#threads] [skipSEQ] [skipPARA]
 *         e.g., ./kde_omp 1 data/Points.csv data/Mask.asc 4 0 0
 *
*/
...
int main(int argc, char *argv[]){
	...
	if(SKIPSEQ == 0){
		...
		EdgeCorrectionWeightsExact(Points, bandWidths, Mask, edgeWeights);
...
//////////////////////////  START OF PARALLEL //////////////////////////
	if(SKIPPARA == 0){
		...
		if(Hoption == 2){
			...
			printf("h0: %.5f alpha0: %.5f Lmax: %.5f\n", h0, alpha0, logL);
	...
	// KDE
	#pragma omp parallel for num_threads(NUM_THREADS) private(p_x, p_y, cell_x, cell_y, p_w, e_w)
		for (int row = 0; row < nRows; row++){
			cell_y = ROW_TO_YCOORD(row, nRows, yLLCorner, cellSize);
			for (int col = 0; col < nCols; col++){
				cell_x = COL_TO_XCOORD(col, xLLCorner, cellSize);
...
void EdgeCorrectionWeightsExact(SamplePoints Points, float *hs, AsciiRaster Ascii, float *weights, int nThreads){
	...
	float ew;
	float h2;
#pragma omp parallel for num_threads(nThreads) private(p_x, p_y, cell_x, cell_y, ew, h2)
	for (int p = 0; p < Points.numberOfPoints; p++){
...
void EdgeCorrectionWeightsExact(SamplePoints Points, float h, AsciiRaster Ascii, float *weights, int nThreads){
	...
	float ew;
	float h2 = h * h;
#pragma omp parallel for num_threads(nThreads) private(p_x, p_y, cell_x, cell_y, ew) firstprivate(h2)
	for (int p = 0; p < Points.numberOfPoints; p++){
		//printf("%6d / %6d\n", p, Points.numberOfPoints);
		p_x = Points.xCoordinates[p];
		p_y = Points.yCoordinates[p];
